...
BASELINE = {
    ...
    "регистр запечён в текст": 245,
    "цветной эмодзи": 0,
    "долг переноса в USS": 167,
...
UNPORTABLE = {
    "backdrop-filter":   "нет: filter размывает САМ элемент, не фон под ним. "
                         "Цена: RenderTexture сцены + свой шейдер, либо отказ от матового стекла",
    ...
    "drop-shadow":       "нет функции drop-shadow и нет box-shadow, но text-shadow ЕСТЬ. "
                         "Цена для теней коробок: 9-slice текстура либо шейдер",
    "gap":               "нет (ни gap, ни column-gap/row-gap). Цена: margin на детях",
    "box-shadow":        "нет. Это ЦЕНТРАЛЬНАЯ потеря: на ней стоит рампа высот. "
                         "Цена: 9-slice текстура свечения на каждую ступень, либо "
                         "рампа переносится на границу и заливку без свечения",
...
def check_uss_debt() -> None:
    ...
    отчётом точнее, чем переездом, — и держится храповиком.
...
def main() -> int:
    ...
    # Проверки, для которых храповик не заведён, — всегда ошибка: молча
    ...
    if slack:
        ...
        print("  Подтяните числа в BASELINE, иначе храповик даст откатиться назад.")
