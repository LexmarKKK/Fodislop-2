...
// Ground-декали — пыль по полу: только осветляют и лишь на треть.
static const float GROUND_DECAL_STRENGTH = 0.35;
...
// руды), серый 0.5 нейтрален — и сила своя.
static const float STONE_DECAL_STRENGTH = 0.7;
...
float3 ApplyTerrainDecal(float3 baseColor, float2 localUV, float packedPlacement)
{
    ...
    if (useStone)
    {
        ...
        return lerp(baseColor, hardLight, decal.a * STONE_DECAL_STRENGTH);
    }

    float3 screen = 1.0 - (1.0 - baseColor) * (1.0 - decal.rgb);
    return lerp(baseColor, screen, decal.a * GROUND_DECAL_STRENGTH);
}
