...
/// старте; недостающие маркеры (они регистрируются лениво) дооткрываются
/// только после отчёта, когда кадр уже всё равно провис. Полный перебор
/// остался по запросу — вкладка «Всплески» окна «Разбор кадра».
public sealed class FrameStallMonitor : IStartable, ITickable, IDisposable
{
    ...
    private void OpenMissingProbes()
    {
        foreach (FrameProbe probe in _probes)
        {
            if (!probe.Available)
