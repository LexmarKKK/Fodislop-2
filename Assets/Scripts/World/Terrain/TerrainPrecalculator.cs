...
namespace Fodinae.Scripts.World.Terrain
{
    public class TerrainPrecalculator
    {
            ...
            if (vxLen > 0 || vyLen > 0)
            {
                if (vxLen > 0)
                {
                    for (int x = vxStart; x < vxStart + vxLen; x++)
                        for (int y = 0; y < gh; y++) CalculateVertexNode(cellCache, x, y);
                }
                ...
                if (vyLen > 0 && vxLen < gw)
                {
                    ...
                    if (xStart < xEnd)
                    {
                        for (int y = vyStart; y < vyStart + vyLen; y++)
                            for (int x = xStart; x < xEnd; x++) CalculateVertexNode(cellCache, x, y);
                    }
                }
            }

            int cxStart = 0, cxLen = 0, cyStart = 0, cyLen = 0;
            if (dx > 0) { cxStart = meshWidth - dx; cxLen = dx; }
            else if (dx < 0) { cxStart = 0; cxLen = -dx; }

            if (dy > 0) { cyStart = meshHeight - dy; cyLen = dy; }
            ...
            if (cxLen > 0 || cyLen > 0)
            {
                if (cxLen > 0)
                {
                    for (int x = cxStart; x < cxStart + cxLen; x++)
                        for (int y = 0; y < meshHeight; y++) CalculateCellNode(cellCache, x, y);
                }
                ...
                if (cyLen > 0 && cxLen < meshWidth)
                {
                    ...
                    if (xStart < xEnd)
                    {
                        for (int y = cyStart; y < cyStart + cyLen; y++)
                            for (int x = xStart; x < xEnd; x++) CalculateCellNode(cellCache, x, y);
                    }
                }
            }
        }
        ...
        private void CalculateCellNode(TerrainCellCache cellCache, int x, int y)
        {
            ...
            {
                byte m = 0;
                if (cellCache.GetCellData(cx - 1, cy).HasTileGroup && cellCache.GetCellData(cx - 1, cy).TileGroupId == data.TileGroupId) m |= 1 << 0;
                if (cellCache.GetCellData(cx - 1, cy - 1).HasTileGroup && cellCache.GetCellData(cx - 1, cy - 1).TileGroupId == data.TileGroupId) m |= 1 << 1;
                if (cellCache.GetCellData(cx, cy - 1).HasTileGroup && cellCache.GetCellData(cx, cy - 1).TileGroupId == data.TileGroupId) m |= 1 << 2;
                if (cellCache.GetCellData(cx + 1, cy - 1).HasTileGroup && cellCache.GetCellData(cx + 1, cy - 1).TileGroupId == data.TileGroupId) m |= 1 << 3;
                if (cellCache.GetCellData(cx + 1, cy).HasTileGroup && cellCache.GetCellData(cx + 1, cy).TileGroupId == data.TileGroupId) m |= 1 << 4;
                if (cellCache.GetCellData(cx + 1, cy + 1).HasTileGroup && cellCache.GetCellData(cx + 1, cy + 1).TileGroupId == data.TileGroupId) m |= 1 << 5;
                if (cellCache.GetCellData(cx, cy + 1).HasTileGroup && cellCache.GetCellData(cx, cy + 1).TileGroupId == data.TileGroupId) m |= 1 << 6;
                if (cellCache.GetCellData(cx - 1, cy + 1).HasTileGroup && cellCache.GetCellData(cx - 1, cy + 1).TileGroupId == data.TileGroupId) m |= 1 << 7;
            ...
            byte rm = 0;
            bool isR = false;
            if (cellCache.GetCellData(cx, cy + 1).ReliefGroup >= data.ReliefGroup) rm |= 1; else isR = true;
            if (cellCache.GetCellData(cx - 1, cy).ReliefGroup >= data.ReliefGroup) rm |= 2; else isR = true;
            if (cellCache.GetCellData(cx, cy - 1).ReliefGroup >= data.ReliefGroup) rm |= 4; else isR = true;
            if (cellCache.GetCellData(cx + 1, cy).ReliefGroup >= data.ReliefGroup) rm |= 8; else isR = true;

            CellReliefMasks[x, y] = rm;
            CellIsRelief[x, y] = isR;

            byte sm = 0;
            if (MapManager.IsRoundableLoose(cellCache.GetCellData(cx, cy + 1).Type)) sm |= 1;
            if (MapManager.IsRoundableLoose(cellCache.GetCellData(cx - 1, cy).Type)) sm |= 2;
            int bt = (int)cellCache.GetCellData(cx, cy - 1).Type;
            if (MapManager.IsRoundableLoose((CellType)bt) || (bt < 32 || bt > 35)) sm |= 4;
            if (MapManager.IsRoundableLoose(cellCache.GetCellData(cx + 1, cy).Type)) sm |= 8;
